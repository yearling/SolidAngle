...
private:
	...
	void RemapFromSkelMesh(YSkeletalMesh  & SourceSkeletalMesh, YSkeleton& TargetSkeleton);

	/** Cache remapping data if current Asset is a Skeleton, with all compatible Skeletons. */
	void RemapFromSkeleton(YSkeleton const & SourceSkeleton);
};
//USTRUCT()
struct FBoneReference
{
	...
	ENGINE_API bool Initialize(const YSkeleton* Skeleton);
