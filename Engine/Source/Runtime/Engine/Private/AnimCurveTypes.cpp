 ...
 */
void FRawCurveTracks::EvaluateTransformCurveData(YSkeleton * Skeleton, TMap<FName, FTransform>&OutCurves, float CurrentTime, float BlendWeight) const
{
	check (Skeleton);
	// evaluate the curve data at the CurrentTime and add to Instance
...
#endif
FAnimCurveBase * FRawCurveTracks::GetCurveData(YSkeleton::AnimCurveUID Uid, ESupportedCurveType SupportedCurveType /*= FloatType*/)
{
...
DataType * FRawCurveTracks::GetCurveDataImpl(TArray<DataType> & Curves, YSkeleton::AnimCurveUID Uid)
{
...
void FAnimCurveParam::Initialize(YSkeleton* Skeleton)
{
	// Initialize for curve UID
	if (Name != NAME_None)
	{
		UID = Skeleton->GetUIDByName(YSkeleton::AnimCurveMappingName, Name);
	}
	else
	{
