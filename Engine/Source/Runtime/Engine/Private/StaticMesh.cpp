...
void FStaticMeshRenderData::Serialize(FArchive& Ar, UStaticMesh* Owner, bool bCooked)
{
	...
	const bool bHasEditorData  = true;
	...
	for (; LODIndex < MaxLODs; ++LODIndex)
	{
		...
		if (Owner->bAutoComputeLODScreenSize)
		{
			...
			else
			{
				...
				const float ViewDistance = CalculateViewDistance(LOD.MaxDeviation, 0.5f);
...
------------------------------------------------------------------------------*/
FStaticMeshLODSettings::FStaticMeshLODSettings()
{
...
void UStaticMesh::Serialize(FArchive& Ar)
{
	...
	if (Ar.UE4Ver() >= VER_UE4_STATIC_MESH_STORE_NAV_COLLISION)
	{
			...
			(NavCollision == nullptr))
		{
			...
			{
				UE_LOG(LogStaticMesh, Warning, TEXT("Serialized NavCollision but it was null (%s) NavCollision will be created dynamicaly at cook time.  Please resave package ."), *GetName()/*, *GetOutermost()->GetPathName()*/);
			}
		}
#endif
	}
	...
	for( int32 CurLODModelIndex = 0; CurLODModelIndex < NumLods; ++CurLODModelIndex )
	{
		...
		switch(result)
		{
			case FLocal::UVCheck_OK:
				InOutAssetsWithValidUVSets.Add( InStaticMesh->GetName() );
			break;
			case FLocal::UVCheck_Bad:
				InOutAssetsWithBadUVSets.Add( InStaticMesh->GetName() );
			break;
			case FLocal::UVCheck_Missing:
				InOutAssetsWithMissingUVSets.Add( InStaticMesh->GetName() );
			break;
			default:
			break;
		}
...
ENGINE_API FString UStaticMesh::GetName()
{
