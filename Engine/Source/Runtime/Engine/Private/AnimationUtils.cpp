...
void FAnimationUtils::BuildSkeletonMetaData(YSkeleton* Skeleton, TArray<FBoneData>& OutBoneData)
{
...
void FAnimationUtils::ComputeCompressionError(const UAnimSequence* AnimSeq, const TArray<FBoneData>& BoneData, AnimationErrorStats& ErrorStats)
{
	...
	if (AnimSeq->NumFrames > 0)
	{
		...
		YSkeleton* Skeleton = AnimSeq->GetSkeleton();
		check ( Skeleton );
...
static inline UAnimCompress* ConstructDefaultCompressionAlgorithm()
{
	...
	UAnimCompress* CompressionAlgorithmClass = AnimSettings->DefaultCompressionAlgorithm;
	...
	UAnimCompress* NewAlgorithm = CompressionAlgorithmClass;
		...
		if( bTryAlternateCompressor && !AnimSeq->bDoNotOverrideCompression )
		{
...

			{
				...
				if( bTryPerTrackBitwiseCompression )
				{
					...
					UAnimCompress_PerTrackCompression* PerTrackCompressor = new UAnimCompress_PerTrackCompression();
				...
				if( bTryFixedBitwiseCompression )
				{
					...
					UAnimCompress_BitwiseCompressOnly* BitwiseCompressor = new UAnimCompress_BitwiseCompressOnly();
...
// 					TRYCOMPRESSION(BitwiseACF_Fixed32,BitwiseCompressor);
				...
				if( bTryFixedBitwiseCompression && bTryIntervalKeyRemoval )
				{
					...
					UAnimCompress_RemoveEverySecondKey* RemoveEveryOtherKeyCompressor = new UAnimCompress_RemoveEverySecondKey();
...
// 							TRYCOMPRESSION(HalfEvenACF_Fixed32,RemoveEveryOtherKeyCompressor);
				...
				if( bTryLinearKeyRemovalCompression && AnimSeq->NumFrames > 1 )
				{
					...
					UAnimCompress_RemoveLinearKeys* LinearKeyRemover = new UAnimCompress_RemoveLinearKeys();
					{
						// Try ACF_Float96NoW
...
// 						PerTrackCompressor->MaxAngleDiff = 0.025;
				...
				if( bTryPerTrackBitwiseCompression && bTryIntervalKeyRemoval)
				{
					...
					if (AnimSeq->NumFrames >= 3)
					{
						...
						UAnimCompress_PerTrackCompression* NewPerTrackCompressor = new UAnimCompress_PerTrackCompression();
...
static void GetBindPoseAtom(FTransform &OutBoneAtom, int32 BoneIndex, YSkeleton *Skeleton)
{
