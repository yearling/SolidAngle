...
struct FBitSet
{
	...
	static FORCEINLINE uint32 GetAndClearNextBit(uint32& Mask)
	{
		...
		const uint32 BitIndex = YMath::FloorLog2(LowestBitMask);
		Mask ^= LowestBitMask;
		return BitIndex;
	}
};
...
public:
	...
	*/
	friend FArchive& operator<<(FArchive& Ar, TBitArray& BitArray)
	{
		...
		const int32 NumDWORDs = YMath::DivideAndRoundUp(BitArray.NumBits, NumBitsPerDWORD);
	...
	int32 Add(const bool Value)
	{
		...
		if (bReallocate)
		{
			...
			const uint32 MaxDWORDs = AllocatorInstance.CalculateSlackGrow(
				YMath::DivideAndRoundUp(NumBits, NumBitsPerDWORD),
				YMath::DivideAndRoundUp(MaxBits, NumBitsPerDWORD),
				sizeof(uint32)
			);
	...
	*/
	void Empty(int32 ExpectedNumBits = 0)
	{
		...
		ExpectedNumBits = YMath::DivideAndRoundUp(ExpectedNumBits, NumBitsPerDWORD) * NumBitsPerDWORD;
	...
	*/
	...
	{
		return YMath::DivideAndRoundUp(MaxBits, NumBitsPerDWORD) * sizeof(uint32);
	}
	...
	void CountBytes(FArchive& Ar)
	{
		Ar.CountBytes(
			...
			YMath::DivideAndRoundUp(MaxBits, NumBitsPerDWORD) * sizeof(uint32)
		);
	}
	...
	*/
	int32 Find(bool bValue) const
	{
		...
		const int32 DwordCount = YMath::DivideAndRoundUp(LocalNumBits, NumBitsPerDWORD);
		int32 DwordIndex = 0;
		...
		if (DwordIndex < DwordCount)
		{
			...
			ASSUME(Bits != 0);
			const int32 LowestBitIndex = YMath::CountTrailingZeros(Bits) + (DwordIndex << NumBitsPerDWORDLogTwo);
	...
	*/
	...
	FORCEINLINE FBitReference AccessCorrespondingBit(const FRelativeBitReference& RelativeReference)
	{
		...
		checkSlow(((uint32)RelativeReference.DWORDIndex + 1) * NumBitsPerDWORD - 1 - YMath::CountLeadingZeros(RelativeReference.Mask) < (uint32)NumBits);
		return FBitReference(
...
private:
	...
	{
		const int32 PreviousNumDWORDs = YMath::DivideAndRoundUp(PreviousNumBits, NumBitsPerDWORD);
		const int32 MaxDWORDs = YMath::DivideAndRoundUp(MaxBits, NumBitsPerDWORD);
...
private:
	...
	void FindFirstSetBit()
	{
		...
		CurrentBitIndex = BaseBitIndex + NumBitsPerDWORD - 1 - YMath::CountLeadingZeros(this->Mask);
...
public:
	...
	*/
	...
	void Empty(int32 Slack = 0)
	{
		...
		Slack = YMath::DivideAndRoundUp(Slack, NumBitsPerDWORD) * NumBitsPerDWORD;
		// If the expected number of bits doesn't match the allocated number of bits, reallocate.
		if (MaxBits != Slack)
		{
			MaxBits = Slack;
