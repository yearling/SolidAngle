...
struct YGenericPlatformTypes
{
	...
	typedef signed short int	int16;		// 16-bit singed.
	...
	typedef uint8				CHAR8;		// An 8-bit character type      - In-memory only.  8-bit fixed-width. char8_t 
	typedef uint16				CHAR16;		// A 16-bit character type      - In-memory only.  16-bit fixed-width, char16_t
	typedef uint32				CHAR32;		// A 32-bit character type      - In-memory only.  32-bit fixed-width, char32_t
