...
DECLARE_YNAME_STAT( TEXT( "Memory Snapshot" ), STAT_Memory_Snapshot, STATGROUP_MemoryProfiler );
...
FStatsMallocProfilerProxy::FStatsMallocProfilerProxy( YMalloc* InMalloc ) 
	: UsedMalloc( InMalloc )
	, bEnabled(false)
	, bWasEnabled(false)
{
...
void FStatsMallocProfilerProxy::InitializeStatsMetadata()
{
	...
	const YName NameAllocPtr = GET_STATYNAME(STAT_Memory_AllocPtr);
	const YName NameReallocPtr = GET_STATYNAME(STAT_Memory_ReallocPtr);
	const YName NameFreePtr = GET_STATYNAME(STAT_Memory_FreePtr);
	const YName NameAllocSize = GET_STATYNAME(STAT_Memory_AllocSize);
	const YName NameOperationSequenceTag = GET_STATYNAME(STAT_Memory_OperationSequenceTag);

	GET_STATYNAME(STAT_Memory_AllocPtr_Calls);
	GET_STATYNAME(STAT_Memory_ReallocPtr_Calls);
	GET_STATYNAME(STAT_Memory_FreePtr_Calls);

	GET_STATYNAME(STAT_Memory_AllocPtr_Mem);
	GET_STATYNAME(STAT_Memory_FreePtr_Mem);
}
