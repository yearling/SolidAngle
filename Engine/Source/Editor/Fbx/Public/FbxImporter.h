...
struct FBXImportOptions
{
	...
	YVector ImportTranslation;
	YRotator ImportRotation;
	...
	YString BaseColorName;
	YString BaseDiffuseTextureName;
	YString BaseEmissiveColorName;
	YString BaseNormalTextureName;
	YString BaseEmmisiveTextureName;
	YString BaseSpecularTextureName;
	...
	TArray<YString> MaterialCurveSuffixes;
...
public:
	...
	uint64 UniqueId;
	YString Name;
	TMap<YString, FFbxAnimPropertyHandle> NodeProperties;

	uint64 AttributeUniqueId;
	FbxNodeAttribute::EType AttributeType;
	TMap<YString, FFbxAnimPropertyHandle> AttributeProperties;
};
...
public:
	...
	//Conversion API
	UNREALED_API void GetConvertedTransformCurveData(const YString& NodeName, FInterpCurveFloat& TranslationX, FInterpCurveFloat& TranslationY, FInterpCurveFloat& TranslationZ,
...
public:
	...
	static YVector ConvertPos(FbxVector4 Vector);
	static YVector ConvertDir(FbxVector4 Vector);
	static YRotator ConvertEuler(FbxDouble3 Euler);
	static YVector ConvertScale(FbxDouble3 Vector);
	static YVector ConvertScale(FbxVector4 Vector);
	static YRotator ConvertRotation(FbxQuaternion Quaternion);
	static YVector ConvertRotationToFVect(FbxQuaternion Quaternion, bool bInvertRot);
	static YQuat ConvertRotToQuat(FbxQuaternion Quaternion);
	...
	static YTransform ConvertTransform(FbxAMatrix Matrix);
	static YMatrix ConvertMatrix(FbxAMatrix Matrix);

	/*
	 * Convert fbx linear space color to sRGB YColor
	 */
	static YColor ConvertColor(FbxDouble3 Color);

	static FbxVector4 ConvertToFbxPos(YVector Vector);
	static FbxVector4 ConvertToFbxRot(YVector Vector);
	static FbxVector4 ConvertToFbxScale(YVector Vector);
...
//FBXImportOptions* GetImportOptions( class FFbxImporter* FbxImporter, UFbxImportUI* ImportUI, bool bShowOptionDialog, bool bIsAutomated, const YString& FullPath, bool& OutOperationCanceled, bool& OutImportAll, bool bIsObjFormat, bool bForceImportType = false, EFBXImportType ImportType = FBXIT_StaticMesh );
FBXImportOptions* GetImportOptions( class FFbxImporter* FbxImporter, bool bShowOptionDialog, bool bIsAutomated, const YString& FullPath, bool& OutOperationCanceled, bool& OutImportAll, bool bIsObjFormat, bool bForceImportType = false, EFBXImportType ImportType = FBXIT_StaticMesh );
void ApplyImportUIToImportOptions( FBXImportOptions& InOutImportOptions);
...
public:
	 ...
	 */
	int32 GetImportType(const YString& InFilename);
	 ...
	 */
	bool GetSceneInfo(YString Filename, FbxSceneInfo& SceneInfo, bool bPreventMaterialNameClash = false);
	 ...
	 * @return bool
	 */
	bool OpenFile( YString Filename, bool bParseStatistics, bool bForSceneInfo = false );
	 ...
	 * @return bool
	 */
	bool ImportFile(YString Filename, bool bPreventMaterialNameClash = false);
	 ...
	 */
	UNREALED_API bool ImportFromFile(const YString& Filename, const YString& Type, bool bPreventMaterialNameClash = false);
...
protected:
	...
	struct FFbxMaterial
	{
		...
		YString GetName() const { return FbxMaterial ? ANSI_TO_TCHAR( FbxMaterial->GetName() ) : TEXT("None"); }
	};
	...
	YString ErrorMessage;
	// base path of fbx file
	YString FileBasePath;
	...
	bool BuildStaticMeshFromGeometry(FbxNode* Node, UStaticMesh* StaticMesh, TArray<FFbxMaterial>& MeshMaterials, int LODIndex, FRawMesh& RawMesh,
									 EVertexColorImportOption::Type VertexColorImportOption, const TMap<YVector, YColor>& ExistingVertexColorData, const YColor& VertexOverrideColor);
	...
	*/
	...
	bool CreateAndLinkExpressionForMaterialProperty(	FbxSurfaceMaterial& FbxMaterial,
														...
														TArray<YString>& UVSet,
														const YVector2D& Location );
	/**
	...
	*/
	 ...
	 */
	int32 CreateNodeMaterials(FbxNode* FbxNode, TArray<UMaterialInterface*>& outMaterials, TArray<YString>& UVSets, bool bForSkeletalMesh);
	 ...
	 */
	YString GetMaterialFullName(FbxSurfaceMaterial& FbxMaterial);
	 ...
	 * @param outUVSets
	 */
	void CreateUnrealMaterial(FbxSurfaceMaterial& FbxMaterial, TArray<UMaterialInterface*>& OutMaterials, TArray<YString>& UVSets, bool bForSkeletalMesh);
	...
	*/
	UMaterialInterface* FindExistingUnrealMaterial(const YString& BasePath, const YString& MaterialName);
...
public:
	...
	//static UObject* CreateAssetOfClass(UClass* AssetClass, YString ParentPackageName, YString ObjectName, bool bAllowReplace = false);

	/* Templated function to create an asset with given package and name */
	template< class T> 
	static T * CreateAsset( YString ParentPackageName, YString ObjectName, bool bAllowReplace = false )
	{
...
protected:
	 ...
	 */
	UInterpGroupInst* CreateMatineeGroup(AMatineeActor* InMatineeActor, AActor* Actor, YString GroupName);
	/**
...
private:
	 ...
	 */
	bool ImportCurveToAnimSequence(class UAnimSequence * TargetSequence, const YString& CurveName, const FbxAnimCurve* FbxCurve, int32 CurveFlags,const FbxTimeSpan AnimTimeSpan, const float ValueScale = 1.f) const;
};
