...
public:
	 ...
	 */
	void Initialize(const FConfigFile& IniFile);
...
private:
	...
	/** Corresponds to YStaticMesh::UseFullPrecisionUVs. if true then 32 bit UVs are used */
...
struct FStaticMeshLODResources
{
	...
	void InitResources(YStaticMesh* Parent);
	 ...
	 */
	void InitVertexFactory(FLocalVertexFactory& InOutVertexFactory, YStaticMesh* InParentMesh, bool bInOverrideColorVertexBuffer);
};
...
public:
	 ...
	 */
	void Cache(YStaticMesh* Owner, const FStaticMeshLODSettings& LODSettings);
#endif // #if WITH_EDITORONLY_DATA

	/** Serialization. */
	void Serialize(FArchive& Ar, YStaticMesh* Owner, bool bCooked);

	/** Initialize the render resources. */
	void InitResources(YStaticMesh* Owner);
...
private:
	...
	/** Resolve all per-section settings. */
	 void ResolveSectionInfo(YStaticMesh* Owner);
#endif // #if WITH_EDITORONLY_DATA
};
...
//	FStaticMeshComponentRecreateRenderStateContext( YStaticMesh* InStaticMesh, bool InUnbuildLighting = true, bool InRefreshBounds = false )
//		: bUnbuildLighting( InUnbuildLighting ),
//		  bRefreshBounds( InRefreshBounds )
//	{
//		for ( TObjectIterator<YStaticMeshComponent> It;It;++It )
//		{
...
//		// The static mesh scene proxies reference the YStaticMesh, and this ensures that they are cleaned up before the YStaticMesh changes.
...
//		{
//			YStaticMeshComponent* Component = StaticMeshComponents[ ComponentIndex ];
//			if ( bUnbuildLighting )
//			{
...
//private:
//
//	TArray<YStaticMeshComponent*> StaticMeshComponents;
...
//
//	/** Initialization constructor. */
//	FStaticMeshSceneProxy(YStaticMeshComponent* Component, bool bCanLODsShareStaticLighting);
//
...
//
//		/** Initialization constructor. */
//		FLODInfo(const YStaticMeshComponent* InComponent, int32 InLODIndex, bool bCanLODsShareStaticLighting);
//
