...
public:
				YAllocator2D( uint32 Width, uint32 Height );
				~YAllocator2D();
	...
	bool		Find( FRect& Rect, const YAllocator2D& Other );
	bool		Test( FRect Rect, const YAllocator2D& Other );
	void		Alloc( FRect Rect, const YAllocator2D& Other );
...
FORCEINLINE uint64 YAllocator2D::GetBit( uint32 x, uint32 y ) const
{
	return Bits[ (x >> 6) + y * Pitch ] & ( 1ull << ( x & 63 ) );
}

FORCEINLINE void YAllocator2D::SetBit( uint32 x, uint32 y )
{
	Bits[ (x >> 6) + y * Pitch ] |= ( 1ull << ( x & 63 ) );
}

inline bool YAllocator2D::Test( FRect Rect )
{
...
inline bool YAllocator2D::Test( FRect Rect, const YAllocator2D& Other )
{
