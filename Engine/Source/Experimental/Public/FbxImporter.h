...
#include "FbxAnimSequenceImportData.h"
...
public:
	...
	//Name API
	 void GetAnimatedNodeNameArray(TArray<FString> &AnimatedNodeNames) const;
	 void GetNodeAnimatedPropertyNameArray(const FString &NodeName, TArray<FString> &AnimatedPropertyNames) const;
	 void GetCurveData(const FString& NodeName, const FString& PropertyName, int32 ChannelIndex, int32 CompositeIndex, FInterpCurveFloat& CurveData, bool bNegative) const;
	 void GetBakeCurveData(const FString& NodeName, const FString& PropertyName, int32 ChannelIndex, int32 CompositeIndex, TArray<float>& CurveData, float PeriodTime, float StartTime = 0.0f, float StopTime= -1.0f, bool bNegative = false) const;

	//Handle API
	 void GetAllNodePropertyCurveHandles(const FString& NodeName, const FString& PropertyName, TArray<FFbxAnimCurveHandle> &PropertyCurveHandles) const;
	 void GetCurveHandle(const FString& NodeName, const FString& PropertyName, int32 ChannelIndex, int32 CompositeIndex, FFbxAnimCurveHandle &CurveHandle) const;
	 void GetCurveData(const FFbxAnimCurveHandle &CurveHandle, FInterpCurveFloat& CurveData, bool bNegative) const;
	 void GetBakeCurveData(const FFbxAnimCurveHandle &CurveHandle, TArray<float>& CurveData, float PeriodTime, float StartTime = 0.0f, float StopTime = -1.0f, bool bNegative = false) const;

	//Conversion API
	 void GetConvertedTransformCurveData(const FString& NodeName, FInterpCurveFloat& TranslationX, FInterpCurveFloat& TranslationY, FInterpCurveFloat& TranslationZ,
...
public:
	 ...
	 */
	bool GetSceneInfo(FString Filename, UnFbx::FbxSceneInfo& SceneInfo, bool bPreventMaterialNameClash = false);
	...
	* @return UObject* the new Unreal mesh object
	*/
	YSkeletalMesh* ReimportSkeletalMesh(YSkeletalMesh* Mesh, UFbxSkeletalMeshImportData* TemplateImportData, uint64 SkeletalMeshFbxUID = 0xFFFFFFFFFFFFFFFF, TArray<FbxNode*> *OutSkeletalMeshArray = nullptr);
	 ...
	 */
	bool ImportAnimation(YSkeleton* Skeleton, UAnimSequence* DestSeq, const FString& FileName, TArray<FbxNode*>& SortedLinks, TArray<FbxNode*>& NodeArray, FbxAnimStack* CurAnimStack, const int32 ResampleRate, const FbxTimeSpan AnimTimeSpan);
	/**
	 ...
	 */
	void ImportFbxMorphTarget(TArray<FbxNode*> &SkelMeshNodeArray, YSkeletalMesh* BaseSkelMesh, UObject* Parent, int32 LODIndex);
	 ...
	 */
	bool ImportSkeletalMeshLOD(YSkeletalMesh* InSkeletalMesh, YSkeletalMesh* BaseSkeletalMesh, int32 DesiredLOD, bool bNeedToReregister = true, TArray<UActorComponent*>* ReregisterAssociatedComponents = NULL);
	...
	*/
	 void FillFbxMeshArray(FbxNode* Node, TArray<FbxNode*>& outMeshArray, UnFbx::FFbxImporter* FFbxImporter);
	...
	*/
	 void FillFbxMeshAndLODGroupArray(FbxNode* Node, TArray<FbxNode*>& outLODGroupArray, TArray<FbxNode*>& outMeshArray);
	...
	*/
	 ...
	 * @param OutSortedLinks
	 */
	// 1. 首先找到ClusterArray中的link中的RootLink;
	// 2. 遍历RootLink来获取整个骨骼链
...
private:
	...
	*/
	void FillLastImportMaterialNames(TArray<FName> &LastImportedMaterialNames, YSkeletalMesh* BaseSkelMesh, TArray<FName> *OrderedMaterialNames);
	...
	*/
	 ...
	 */
	void ImportMorphTargetsInternal( TArray<FbxNode*>& SkelMeshNodeArray, YSkeletalMesh* BaseSkelMesh, UObject* Parent, int32 LODIndex );
	...
	*/
	void InsertNewLODToBaseSkeletalMesh(YSkeletalMesh* InSkeletalMesh, YSkeletalMesh* BaseSkeletalMesh, int32 DesiredLOD);
...
protected:
	...
	*/
	...
	//
public:
	 void PopulateAnimatedCurveData(FFbxCurvesAPI &CurvesAPI);
...
public:
	 ...
	 */
	 bool HasUnknownCameras( AMatineeActor* InMatineeActor ) const;
	 ...
	 */
	 bool ImportMatineeSequence(AMatineeActor* InMatineeActor);
...
protected:
	 ...
	 */
	void FillAndVerifyBoneNames(YSkeleton* Skeleton, TArray<FbxNode*>& SortedLinks, TArray<FName> & OutRawBoneNames, FString Filename);
	/**
