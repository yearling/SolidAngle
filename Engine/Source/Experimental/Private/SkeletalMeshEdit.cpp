...
void UnFbx::FFbxImporter::FillAndVerifyBoneNames(YSkeleton* Skeleton, TArray<FbxNode*>& SortedLinks, TArray<FName>& OutRawBoneNames, FString Filename)
{
	...
	{
		// warn user
		AddTokenizedErrorMessage(FTokenizedMessage::Create(EMessageSeverity::Warning, FText::Format(LOCTEXT("FBXImport_MissingBone", "The following bones exist in the imported animation, but not in the Skeleton asset.  Any animation on these bones will not be imported: \n\n {0}"), /*FText::FromString(Skeleton->GetName()),*/ FText::FromString(BoneNames) )), FFbxErrors::Animation_MissingBones);
	}
}
//-------------------------------------------------------------------------
//
...
bool UnFbx::FFbxImporter::ImportCurveToAnimSequence(class UAnimSequence * TargetSequence, const FString& CurveName, const FbxAnimCurve* FbxCurve, int32 CurveFlags,const FbxTimeSpan AnimTimeSpan, const float ValueScale/*=1.f*/) const
{
	if (TargetSequence && FbxCurve)
	{
		...
		YSkeleton* Skeleton = TargetSequence->GetSkeleton();
		const FSmartNameMapping* NameMapping = Skeleton->GetSmartNameContainer(YSkeleton::AnimCurveMappingName);
		...
		FSmartName NewName;
		Skeleton->AddSmartNameAndModify(YSkeleton::AnimCurveMappingName, Name, NewName);
		...
		const FSmartNameMapping* Mapping = Skeleton->GetSmartNameContainer(YSkeleton::AnimCurveMappingName);
...
bool UnFbx::FFbxImporter::ImportAnimation(YSkeleton* Skeleton, UAnimSequence * DestSeq, const FString& FileName, TArray<FbxNode*>& SortedLinks, TArray<FbxNode*>& NodeArray, FbxAnimStack* CurAnimStack, const int32 ResampleRate, const FbxTimeSpan AnimTimeSpan)
{
	...
	YSkeleton* MySkeleton = DestSeq->GetSkeleton();
	check(MySkeleton);
	...
	// import animation
	{
		...
		UFbxAnimSequenceImportData* TemplateData = dynamic_cast<UFbxAnimSequenceImportData*>(DestSeq->AssetImportData);
