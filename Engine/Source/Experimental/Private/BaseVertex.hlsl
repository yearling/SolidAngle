...
matrix globalMatrix0;
matrix globalMatrix1;
matrix globalMatrix2;
float  globalFloat0;
...
VS_OUTPUT VSMain( VS_INPUT Input )
{
	...
	matWVP = mul(matWVP, globalMatrix0);
	matWVP = mul(matWVP, globalMatrix1);
	matWVP = mul(matWVP, globalMatrix2);
	Output.vPosition = mul( Input.vPosition, matWVP);
	Output.vNormal = normalize(mul(Input.vNormal, (float3x3) g_world));
	Output.vTexcoord = Input.vTexcoord;
	Output.vColor = Input.vColor;
	Output.vColor *= globalFloat0;
	Output.vColor += LocalArray[5];
	return Output;
}
