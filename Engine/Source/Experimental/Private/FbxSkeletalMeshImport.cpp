...
extern ExistingSkelMeshData* SaveExistingSkelMeshData(YSkeletalMesh* ExistingSkelMesh, bool bSaveMaterials);
extern void RestoreExistingSkelMeshData(ExistingSkelMeshData* MeshData, YSkeletalMesh* SkeletalMesh);
...
void UnFbx::FFbxImporter::FillLastImportMaterialNames(TArray<FName> &LastImportedMaterialNames, YSkeletalMesh* BaseSkelMesh, TArray<FName> *OrderedMaterialNames)
{
#if 0
...
YSkeletalMesh* UnFbx::FFbxImporter::ImportSkeletalMesh(UObject* InParent, TArray<FbxNode*>& NodeArray, const FName& Name,  UFbxSkeletalMeshImportData* TemplateImportData, int32 LodIndex, bool* bCancelOperation, TArray<FbxShape*> *FbxShapeArray, FSkeletalMeshImportData* OutData, bool bCreateRenderData, TArray<FName> *OrderedMaterialNames)
{
	...
	YSkeletalMesh* ExistingSkelMesh = nullptr;
	//if ( !FbxShapeArray  )
	//{
	...
	//	//ExistingSkelMesh = Cast<YSkeletalMesh>(ExistingObject);

	//	if (!ExistingSkelMesh/* && ExistingObject*/)
	//	{
	//		//AddTokenizedErrorMessage(FTokenizedMessage::Create(EMessageSeverity::Warning, FText::Format(LOCTEXT("FbxSkeletaLMeshimport_OverlappingName", "Same name but different class: '{0}' already exists"), FText::FromString(TEXT(""))), FFbxErrors::Generic_SameNameAssetExists);
	//		return NULL;
	//	}
	//}
	...
	//					
	//					FARFilter ARFilter;
	//					ARFilter.ClassNames.Add(*YSkeletalMesh::StaticClass()->GetName());
	...
	//							const YSkeletalMesh* ExtraSkeletalMesh = Cast<YSkeletalMesh>(CurAssetData.GetAsset());
...
YSkeletalMesh* UnFbx::FFbxImporter::ReimportSkeletalMesh(YSkeletalMesh* Mesh, UFbxSkeletalMeshImportData* TemplateImportData, uint64 SkeletalMeshFbxUID, TArray<FbxNode*> *OutSkeletalMeshArray)
{
	...
	if (FbxNodes)
	{
		...
		for (LODIndex = 0; LODIndex < MaxLODLevel; LODIndex++)
		{
			...
			{
				YSkeletalMesh* BaseSkeletalMesh = Cast<YSkeletalMesh>(NewMesh);
				...
				ImportSkeletalMeshLOD( Cast<YSkeletalMesh>(LODObject), BaseSkeletalMesh, LODIndex);
...
void UnFbx::FFbxImporter::InsertNewLODToBaseSkeletalMesh(YSkeletalMesh* InSkeletalMesh, YSkeletalMesh* BaseSkeletalMesh, int32 DesiredLOD)
{
#if 0
...
bool UnFbx::FFbxImporter::ImportSkeletalMeshLOD(YSkeletalMesh* InSkeletalMesh, YSkeletalMesh* BaseSkeletalMesh, int32 DesiredLOD, bool bNeedToReregister, TArray<UActorComponent*>* ReregisterAssociatedComponents)
{
#if 0
...
#endif 
void UnFbx::FFbxImporter::ImportMorphTargetsInternal( TArray<FbxNode*>& SkelMeshNodeArray, YSkeletalMesh* BaseSkelMesh, UObject* InParent, int32 LODIndex )
{
#if 0
...
// Import Morph target
void UnFbx::FFbxImporter::ImportFbxMorphTarget(TArray<FbxNode*> &SkelMeshNodeArray, YSkeletalMesh* BaseSkelMesh, UObject* InParent, int32 LODIndex)
{
#if 0
