...
void FReferenceSkeleton::RebuildRefSkeleton(const YSkeleton* Skeleton, bool bRebuildNameMap)
{
	if (bRebuildNameMap)
	{
...
void FReferenceSkeleton::RemoveDuplicateBones(const UObject* Requester, TArray<FBoneIndexType> & DuplicateBones)
{
	...
	for (int32 BoneIndex = NumBones - 1; BoneIndex >= 0; BoneIndex--)
	{
		...
		else
		{
			...
			UE_LOG(LogAnimation, Warning, TEXT("RemoveDuplicateBones: duplicate bone name (%s) detected for (%s)! Indices: %d and %d. Removing the latter."),
				*BoneName.ToString(), /**GetNameSafe(Requester)*/TEXT("None"), DuplicateBoneIndex, BoneIndex);
	...
	{
		//const YSkeleton* Skeleton = dynamic_cast<YSkeleton*>(Requester);
		//if (!Skeleton)
		//{
		//	if (const YSkeletalMesh* Mesh = dynamic_cast<YSkeletalMesh*>(Requester))
		//	{
