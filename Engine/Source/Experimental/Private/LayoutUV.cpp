...
YLayoutUV::YLayoutUV( YRawMesh* InMesh, uint32 InSrcChannel, uint32 InDstChannel, uint32 InTextureResolution )
	: RawMesh( InMesh )
...
void YLayoutUV::FindCharts( const TMultiMap<int32,int32>& OverlappingCorners )
{
	...
	// Build disjoint set
	YDisjointSet DisjointSet( NumTris );
	for( uint32 i = 0; i < NumIndexes; i++ )
	{
	...
	struct FCompareTris
	{
		YDisjointSet* DisjointSet;

		FCompareTris( YDisjointSet* InDisjointSet )
		...
		for( uint32 Tri = Chart.FirstTri; Tri < Chart.LastTri; Tri++ )
		{
			for( int k = 0; k < 3; k++ )
			{
				...
				if( TranslatedMatches[ Index ] >= 0 )
				{
					...
					YMeshChart& ChartA = Charts[ ChartI ];
					YMeshChart& ChartB = Charts[ ChartJ ];
	...
	for( uint32 Axis = 0; Axis < 2; Axis++ )
	{
		for( int32 i = 0; i < Charts.Num(); i++ )
		{
			...
			for( uint32 Side = 0; Side < 4; Side++ )
			{
				...
				{
					YMeshChart& ChartB = Charts[ ChartA.Join[ Side ] ];
		...
		for( int32 i = 0; i < Charts.Num(); i++ )
		{
			...
			if( ChartA.Join[ Side ^ 0 ] == -1 )
			{
				...
				{
					YMeshChart& ChartB = Charts[ ChartA.Join[ Side ^ 1 ] ];
			...
			else
			{
				...
				YMeshChart& ChartB = Charts[ ChartA.Join[ Side ^ 0 ] ];
				check( ChartB.Join[ Side ^ 1 ] == i );
				check( ChartB.FirstTri != ChartB.LastTri );
			}
		}
...
bool YLayoutUV::FindBestPacking()
{
...
void YLayoutUV::ScaleCharts( float UVScale )
{
	for( int32 i = 0; i < Charts.Num(); i++ )
	{
		YMeshChart& Chart = Charts[i];
	...
	for( int i = 0; i < 1000; i++ )
	{
		...
		{
			YMeshChart& Chart = Charts[ChartIndex];
	...
	{
		FORCEINLINE bool operator()( const YMeshChart& A, const YMeshChart& B ) const
		{
			// Rect area
...
bool YLayoutUV::PackCharts()
{
	...
	for( int32 i = 0; i < Charts.Num(); i++ )
	{
		...
		YAllocator2D::FRect	BestRect = { ~0u, ~0u, ~0u, ~0u };
...
void YLayoutUV::OrientChart( YMeshChart& Chart, int32 Orientation )
{
	switch( Orientation )
	{
	case 0:
...
void YLayoutUV::RasterizeChart( const YMeshChart& Chart, uint32 RectW, uint32 RectH )
{
...
void YLayoutUV::CommitPackedUVs()
{
